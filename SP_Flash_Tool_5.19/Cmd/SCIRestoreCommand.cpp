...
void SCIRestoreCommand::exec(const QSharedPointer<Connection> &conn)
{
    ...
    int ret = getRestoreItem();
    if(S_DONE != ret)
    {
        LOGI("Get restore item failed, error code: %s(%d).", StatusToString(ret), ret);
    ...
    if(S_DONE != ret)
    {
        LOGI("Restore item failed, error code: %s(%d).", StatusToString(ret), ret);
